...
fn get_uudoc_command() -> Command {
    let uudoc_binary = UUDOC_BINARY_PATH.get_or_init(|| {
        let peiosutils_binary = PathBuf::from(env!("CARGO_BIN_EXE_peiosutils"));
        peiosutils_binary.parent().unwrap().join("uudoc")
    });
...
fn test_manpage_peiosutils() {
    ...
    let output_str = String::from_utf8_lossy(&output.stdout);
    assert!(output_str.contains("\n.TH PEIOSUTILS"), "{output_str}");
    assert!(output_str.contains("peiosutils"), "{output_str}");
    assert!(output_str.contains("\n.SH NAME\npeiosutils"), "{output_str}");
}
