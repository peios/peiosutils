...
/// Build the standard error for a peiosutils command deferred on Peios.
///
/// A deferred command is kept in the tree — it still compiles, so it
/// does not bitrot before revival — but its POSIX data model (utmp, the
/// passwd database, …) has no Peios-native backing yet. Running it
/// errors out rather than presenting empty or stale compat-shim data.
/// `awaiting` names the subsystem the command is waiting for.
pub fn deferred_on_peios(awaiting: &str) -> Box<dyn UError> {
    USimpleError::new(1, format!("deferred on Peios — awaiting {awaiting}"))
}

/// Build the standard error for a command that is not applicable on
/// Peios.
///
/// Unlike a deferred command, this one targets a Linux subsystem that
/// Peios replaces outright — SELinux, for one — so there is no Peios
/// equivalent and no planned revival. Its code is kept in the tree, but
/// running it errors out. `because` states what the command targeted.
pub fn not_applicable_on_peios(because: &str) -> Box<dyn UError> {
    USimpleError::new(1, format!("not applicable on Peios — {because}"))
}
