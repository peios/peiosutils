            ...
            match splice(&input, &broker_w, n as usize) {
                ...
                Ok(s @ 1..) => {
                    if splice_exact(&broker_r, &target, s).is_ok() {
                        ...
                        // The drained bytes were moved out of `input` by the
                        // splice above and have now been delivered to
                        // `target`; account for them so the read/write
                        // fallback below sends only the remaining `n` bytes
                        // rather than re-sending a full `n` on top.
                        let drained = drain.len() as u64;
