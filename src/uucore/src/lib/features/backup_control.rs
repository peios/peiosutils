...
///   how the calling utility opens it (`false` for `mv`, `true` for `cp`).
///
/// # Examples
///
/// ```
...
mod tests {
    ...
    /// Which modes can destroy the source is uucore's question, not the
    /// caller's: `existing` falls back to a simple backup when no numbered
    /// backup is present, and so is just as dangerous.
    #[test]
