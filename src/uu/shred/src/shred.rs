...
use uucore::sd_control::creator_sd_from_sddl;
...
/// Whether the calling process can write `path`, per KACS.
///
/// On Peios `access(2)` is a live AccessCheck against the effective
/// token (`W_OK` -> FILE_WRITE_DATA); the POSIX mode bits are not
/// consulted, so this is the honest signal `--force` keys on before
/// deciding whether it needs to grant write via the descriptor.
...
fn wipe_file(
    ...
    // If --force is given and the file is not writable, override its
    // protection so the overwrite can proceed. KACS does not consult
    // st_mode — the POSIX permission bits are decorative; the access
    // authority is the security descriptor — so --force rewrites the
    // DACL rather than chmod-ing. It replaces the DACL wholesale with
    // one granting full access (D:P(A;;FA;;;WD)): the file is about to
    // be destroyed, --force means "override this file's protection",
    // and granting Everyone sidesteps resolving the caller's own token
    // SID (a narrower grant would need libp-token machinery this flag
    // does not warrant). Writing the DACL needs WRITE_DAC, held
    // implicitly by the owner — exactly as GNU --force needs chmod rights.
    if force && !has_write_access(path) {
        creator_sd_from_sddl("D:P(A;;FA;;;WD)")?.apply_to(path)?;
    }
