...
pub fn uumain(_args: impl uucore::Args) -> uucore::error::UResult<()> {
    // who reads the logged-in-user database (utmp). Peios has no native
    // session model yet -- authd/eventd are undecided -- so there is
    // nothing to read. Defer until that subsystem exists; the platform
    // module holds the implementation, kept compiled, for when it lands.
