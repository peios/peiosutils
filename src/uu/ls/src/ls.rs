...
use uucore::{
    ...
    sd_control::{SdDisplay, read_sd_display},
    show, translate,
...
pub fn uu_app() -> Command {
    ...
    // -1 does nothing when the format is already long; unlike
    // --format=single-column it does not override an earlier format,
    // so it cannot use clap's override-with (see Config::from).
    .arg(
...
pub struct PathData<'a> {
    ...
    // Owner SID + explicit-DACL state, read lazily from the SD.
    sd_display: OnceCell<SdDisplay>,
...
impl<'a> PathData<'a> {
    ...
    fn new(
        ...
        let sd_display: OnceCell<SdDisplay> = OnceCell::new();
    ...
    /// The owner SID and explicit-DACL state from the file's Security
    /// Descriptor, read lazily. See [`uucore::sd_control::read_sd_display`].
    fn sd_display(&self) -> &SdDisplay {
        self.sd_display
            .get_or_init(|| read_sd_display(&self.p_buf, self.must_dereference))
    }
...
///
/// ```ignore
/// use pu_ls::{Config, list_with_output, StreamingOutput};
/// use std::path::Path;
///
/// let config = Config::from(&matches)?;
