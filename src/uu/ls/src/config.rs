...
use crate::{
    ...
    display::{Format, IndicatorStyle, LocaleQuoting},
...
impl Config {
    ...
    pub fn from(options: &clap::ArgMatches) -> UResult<Self> {
        ...
        // --full-time implies the long format, and -1 does nothing when the
        // format is already long. Neither resets a format set earlier on the
        // command line, so compare argument indices rather than relying on
        // clap's override-with.
