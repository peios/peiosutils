...
fn display_dir_entry_size(entry: &PathData, config: &Config) -> (usize, usize, usize, usize) {
    if let Some(md) = entry.metadata() {
        ...
        let owner_len = entry.sd_display().owner.as_deref().unwrap_or("?").len();
        (owner_len, size_len, major_len, minor_len)
    } else {
        (0, 0, 0, 0)
    }
}
...
pub fn display_items(
    ...
    if config.format == Format::Long {
        let padding_collection = calculate_padding_collection(items, config);
        ...
        let padding = calculate_padding_collection(items, config);
...
pub fn display_size(size: u64, config: &Config) -> String {
    let formatted = human_readable(size, config.size_format);
    // Plain byte counts are grouped with commas for readability; the
    // human-readable forms (-h / --si) keep their compact units.
    if config.size_format == SizeFormat::Bytes {
        group_digits(&formatted)
    } else {
        formatted
    }
}

/// Insert `,` thousands separators into a run of ASCII digits. A string
/// that is not all digits, or is three digits or fewer, is returned as is.
fn group_digits(s: &str) -> String {
    ...
    for (i, ch) in s.char_indices() {
        // A comma precedes any digit that begins a fresh group of three.
...
/// The mode column's file-type character: `d`/`l`/`p`/`s`/`b`/`c`, or
/// `-` for a regular file.
fn type_char(md: &Metadata) -> char {
    let ft = md.file_type();
    if ft.is_dir() {
        'd'
...
/// The mode column's type character when metadata could not be read.
fn type_char_unknown(item: &PathData) -> char {
...
/// The mode column's executable mark: `x` when a regular file carries
/// an execute bit (set by `chmod +x` / `mkexec`), else `-`.
fn exec_char(md: &Metadata) -> char {
...
/// Writes a single `ls -l` line to `state.display_buf`.
///
/// Peios `ls -l` is not POSIX `ls -l`: it shows no permission bits,
/// because access on Peios is governed by a Security Descriptor, not by
/// the mode. A line carries, in order:
/// * the mode column `[type][x][+]` — file type; `x` if the file is
///   marked executable (`chmod +x` / `mkexec`); `+` if its DACL is
///   inheritance-protected;
/// * the owner SID from the SD ([`PathData::sd_display`]);
/// * the security context, only with `-Z` (config-optional);
/// * the size, or device major/minor ([`display_len_or_rdev`]);
/// * the modification (or `-c`/`-u`) time ([`display_date`]);
/// * the item name ([`display_item_name`]).
///
/// Variable-width columns are padded to the maxima in [`PaddingCollection`].
#[allow(clippy::cognitive_complexity)]
fn display_item_long(
    ...
    if let Some(md) = item.metadata() {
        let sd = item.sd_display();
        // Mode column: file type, executable mark, protected-DACL mark.
        state.display_buf.push(type_char(md) as u8);
        state.display_buf.push(exec_char(md) as u8);
        state
            .display_buf
            .push(if sd.protected_dacl { b'+' } else { b'-' });

        // Owner SID, read from the Security Descriptor.
        state.display_buf.push(b' ');
        state
            .display_buf
            .extend_pad_right(sd.owner.as_deref().unwrap_or("?"), padding.owner);
        ...
        match display_len_or_rdev(md, config) {
            ...
            SizeOrDeviceId::Device(major, minor) => {
                ...
                state.display_buf.extend(b", ");
                state.display_buf.extend_pad_left(&minor, padding.minor);
            }
        }
        ...
        // Metadata unavailable (e.g. a dangling symlink): the type
        // character is all that can be known; everything else is `?`.
        state.display_buf.push(type_char_unknown(item) as u8);
        state.display_buf.extend(b"??");
        state.display_buf.push(b' ');
        state.display_buf.extend_pad_right("?", padding.owner);
...
fn calculate_padding_collection(items: &[PathData], config: &Config) -> PaddingCollection {
    ...
    for item in items {
        ...
        if config.format == Format::Long {
            let context_len = item.security_context(config).len();
            let (owner_len, size_len, major_len, minor_len) =
                display_dir_entry_size(item, config);
            padding_collections.owner = owner_len.max(padding_collections.owner);
            if config.context {
