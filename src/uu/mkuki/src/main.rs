//! mkuki — build a UEFI unified kernel image from Peios boot inputs.
//!
//! The tool takes a PE/COFF EFI stub and appends UKI payload sections:
//! `.cmdline`, `.linux`, and `.initrd`.
...
fn main() -> ExitCode {
    ...
    let cfg = match parse_args(&args) {
        ...
        Err(e) => {
            eprintln!("mkuki: {e}");
            eprintln!(
                "usage: mkuki [--stub PATH] --kernel PATH --initramfs PATH (--cmdline TEXT | --cmdline-file PATH) --out PATH"
            );
...
fn parse_args(args: &[String]) -> Result<Config, String> {
    ...
    while let Some(arg) = args.next() {
        match arg.as_str() {
            "--stub" => stub = Some(path_value(&mut args, "--stub")?),
            "--kernel" => kernel = Some(path_value(&mut args, "--kernel")?),
            "--initramfs" => initramfs = Some(path_value(&mut args, "--initramfs")?),
            "--cmdline" => {
                if cmdline.is_some() {
                    return Err("use only one of --cmdline or --cmdline-file".into());
                }
                cmdline = Some(Cmdline::Literal(value(&mut args, "--cmdline")?));
            }
            "--cmdline-file" => {
                if cmdline.is_some() {
                    return Err("use only one of --cmdline or --cmdline-file".into());
                }
                cmdline = Some(Cmdline::File(path_value(&mut args, "--cmdline-file")?));
            }
            "--out" => out = Some(path_value(&mut args, "--out")?),
    ...
    Ok(Config {
        stub,
        kernel: kernel.ok_or("missing --kernel PATH")?,
        initramfs: initramfs.ok_or("missing --initramfs PATH")?,
        cmdline: cmdline.ok_or("missing --cmdline TEXT or --cmdline-file PATH")?,
        out: out.ok_or("missing --out PATH")?,
    })
}

fn value<'a>(args: &mut impl Iterator<Item = &'a String>, flag: &str) -> Result<String, String> {
    args.next()
        .cloned()
