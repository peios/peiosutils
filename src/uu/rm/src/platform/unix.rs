...
use super::super::{
    InteractiveMode, Options, is_dir_empty, is_readable, is_writable, prompt_descend, remove_file,
...
/// Directory removal prompt, keyed on the directory's KACS read/write
/// access rather than its (decorative) POSIX mode bits.
fn prompt_dir_access(path: &Path, options: &Options) -> bool {
    if options.interactive == InteractiveMode::Never {
        return true;
    }
    ...
    for entry_name in entries {
        ...
        if is_dir {
            ...
            if !child_error
                ...
                && !prompt_dir_access(&entry_path, options)
            {
                continue;
            }
