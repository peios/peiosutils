...
/// Whether the calling process has `mode` access (one of `libc::R_OK`,
/// `W_OK`, or `X_OK`) to `path`.
///
/// On Peios `access(2)` is routed through KACS: it runs a live
/// AccessCheck against the caller's effective token rather than reading
/// the POSIX mode bits, which KACS leaves on the inode as decorative
/// metadata. `W_OK` maps to FILE_WRITE_DATA, `R_OK` to FILE_READ_DATA.
/// This is what makes rm's "write-protected" prompt honest on a system
/// where `st_mode` carries no access authority.
...
/// Whether the caller has write access to `path`, per KACS.
fn is_writable(path: &Path) -> bool {
    has_access(path, libc::W_OK)
}

/// Whether the caller has read access to `path`, per KACS.
...
fn remove_dir_recursive(
    ...
    // Recursive removal goes through secure openat/unlinkat traversal.
...
fn is_root_path(path: &Path) -> bool {
    ...
    uucore::fs::path_is_root_dir(path, true)
}
...
fn remove_dir(path: &Path, options: &Options, progress_bar: Option<&ProgressBar>) -> bool {
    ...
    // Empty-directory removal goes through secure unlinkat traversal.
    // safe_remove_empty_dir returns None only when the path has no file
    // name component, in which case the feedback fallback handles it.
...
fn remove_file(path: &Path, options: &Options, progress_bar: Option<&ProgressBar>) -> bool {
    if prompt_file(path, options) {
        ...
        // Individual file removal goes through secure unlinkat traversal.
        // safe_remove_file returns None only when the path has no file
        // name component, in which case the direct removal below handles it.
...
fn prompt_dir(path: &Path, options: &Options) -> bool {
    ...
    // Only prompt for a directory that exists; handle_writable_directory
    // asks KACS whether it is readable/writable via access(2).
    if fs::metadata(path).is_ok() {
        handle_writable_directory(path, options)
    } else {
        true
    }
}
...
// Decide whether to prompt before removing a directory, keyed on the
// directory's KACS read/write access. access(2) is the honest signal on
// Peios — st_mode permission bits carry no access authority.
fn handle_writable_directory(path: &Path, options: &Options) -> bool {
    let stdin_ok = options.__presume_input_tty.unwrap_or(false) || stdin().is_terminal();
    match (
        stdin_ok,
        is_readable(path),
...
// A symlink is never itself a directory on Linux — the symlink-to-dir
// distinction the Windows VFS draws does not exist here.
