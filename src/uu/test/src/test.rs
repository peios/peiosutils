...
/// Probe access via access(2). On Peios this routes through KACS as a
/// live AccessCheck against the effective token (R_OK/W_OK/X_OK map to
/// FILE_READ_DATA/FILE_WRITE_DATA/FILE_EXECUTE), rather than deriving
/// access from POSIX uid/gid and mode bits, which KACS leaves as
/// decorative metadata.
fn has_access(path: &OsStr, mode: libc::c_int) -> bool {
...
fn path(path: &OsStr, condition: &PathCondition) -> bool {
    ...
    match condition {
        ...
        PathCondition::Readable => has_access(path, libc::R_OK),
        ...
        PathCondition::Writable => has_access(path, libc::W_OK),
        PathCondition::Executable => has_access(path, libc::X_OK),
    }
}
