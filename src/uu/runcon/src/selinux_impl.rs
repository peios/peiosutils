...
//! The original SELinux `runcon` implementation, preserved verbatim
//! behind the off-by-default `feat_selinux` cargo feature.
//!
//! It is dormant on Peios — the crate root's `uumain` errors out
//! unconditionally (Peios's LSM is KACS, not SELinux) — and retained
//! only as a reference. See the crate-root module documentation.
...
/// The SELinux `runcon` entry point, as it was before Peios. Unused —
/// the crate's real `uumain` errors out — and retained for reference.
