...
impl UNameOutput {
    ...
    pub fn new(opts: &Options) -> UResult<Self> {
        ...
        // The operating system is Peios. -s ("kernel name") still reports
        // the kernel itself ("Linux"); -o names the OS built on top of it.
        let os = (opts.os || opts.all).then(|| OsString::from("Peios"));
