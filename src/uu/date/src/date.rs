...
/// System call to set the realtime clock.
/// See here for more:
/// `<https://linux.die.net/man/3/clock_settime>`
///
/// Enforcement (whether the caller may write `CLOCK_REALTIME`) lives in the
/// kernel — Peios does not pre-check a privilege in userspace. An `EPERM`
/// from the kernel surfaces as the `date-error-cannot-set-date` message.
