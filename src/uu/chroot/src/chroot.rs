...
pub fn uumain(args: impl uucore::Args) -> UResult<()> {
    ...
    // Peios: --userspec and --groups are filesystem-namespace flags wearing
    // identity-flag clothing. They both presuppose UIDs/GIDs, /etc/passwd,
    // /etc/group, and a kernel mechanism to switch process identity — none
    // of which exist in Peios yet. The flag definitions, parsing, and
    // set_context identity code are kept below so the surface is ready when
    // authd lands, but for now any invocation that actually relies on
    // identity transition gets a loud refusal rather than silent success.
    if options.userspec.is_some() {
        return Err(ChrootError::FlagNotSupported("--userspec").into());
    }
    if options.groups.is_some() {
        return Err(ChrootError::FlagNotSupported("--groups").into());
    }
...
/// Every name-service lookup (`name_to_uid`, `name_to_gid`, `usr2gid`,
/// `supplemental_gids`) happens *before* `enter_chroot`. On glibc those go
/// through NSS, which `dlopen()`s `libnss_*.so.2`; resolving after the
/// `chroot()` would load those modules out of NEWROOT and run attacker code as
/// root when NEWROOT is writable (CVE-2026-35368).
