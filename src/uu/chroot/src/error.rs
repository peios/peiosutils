...
pub enum ChrootError {
    ...
    /// Peios: an identity-transition flag was passed but Peios does not yet
    /// support setuid-style identity changes (waiting on authd + KACS subject
    /// identity). The flag is kept in the surface so existing scripts produce
    /// a useful error rather than silently succeeding.
    #[error("{}", translate!("chroot-error-flag-not-supported", "flag" => _0))]
    FlagNotSupported(&'static str),
}
