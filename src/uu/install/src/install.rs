...
pub fn uumain(_args: impl uucore::Args) -> UResult<()> {
    // `install` deploys files while setting their attributes -- POSIX
    // mode, owner/group, and SELinux context. None of those is Peios's
    // model: mode bits collapse to the executable mark, owner/group
    // becomes a security descriptor, and SELinux is replaced by KACS.
    // Rather than ship a half-translation, the command is deferred until
    // the fuller peiosification pass settles that attribute design.
    // Everything below stays compiled so it does not bitrot before then.
    Err(uucore::error::deferred_on_peios(
        "a Peios-native install attribute model (the executable mark and \
         security descriptors in place of POSIX mode and owner/group)",
    ))
}
