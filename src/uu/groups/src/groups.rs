...
pub fn uumain(args: impl uucore::Args) -> UResult<()> {
    ...
    // projection of the *token's* group set, which carries what authd staples
    // on at mint: Everyone and Local are on every token, whoever the principal
    // is.
    //
    ...
    //
    // Measured on a live image (PEI-206): `groups` gives
    // "Authenticated Users Everyone Administrators Local" where
    // `groups <self>` gives "Authenticated Users Administrators".
    //
    ...
    // and the per-sign-in logon session SID) cannot appear, and SE_GROUP_*
    // attributes have nowhere to go, so a deny-only group is
