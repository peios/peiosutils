...
pub struct Options {
    ...
    /// Security descriptor applied to a file this run creates, from the
    /// `--sd*` flag group. `None` leaves a created file with plain kernel
    /// inheritance; ignored for files that already exist.
    pub creator_sd: Option<CreatorSd>,
}
...
fn touch_file(
    ...
    if let Err(e) = metadata_result {
        ...
        // The file was just created — apply the creator SD, if the
        // --sd* flags requested one (create-then-set; see
        // peios/sd-creation-design.md). On failure remove the file so
        // nothing half-secured is left behind.
        if let Some(sd) = &opts.creator_sd
