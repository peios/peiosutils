...
// pinky is deferred on Peios (see uumain below). Its implementation is
// kept compiled but unused, so allow unused code crate-wide rather than
// scattering per-item attributes through the frozen body.
#![allow(unused)]
...
// pinky is deferred on Peios: its data model — utmp login records plus
// the passwd database and GECOS field — has no Peios-native backing.
// Peios replaces POSIX identity with KACS security descriptors, and a
// "who is logged in" session source does not exist yet (authd is
// undecided). Rather than ship a tool that silently shows nothing (the
// static-musl build stubs utmpx anyway), uumain errors out. The
// platform implementation below is kept compiled, ready to be revived
// against a Peios session model.
#[uucore::main(no_signals)]
pub fn uumain(_args: impl uucore::Args) -> UResult<()> {
    Err(uucore::error::deferred_on_peios(
        "a native Peios session and identity model (utmp and the passwd \
         database are not authoritative on Peios)",
    ))
}
