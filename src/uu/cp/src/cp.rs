...
// The `--preserve` machinery lives in `uucore` so `cp` and `mv` share one
// definition. Re-exported so existing call sites (and `copydir`) keep working.
pub use uucore::preserve::{self, Attributes, Preserve, copy_attributes};
use uucore::{
...
pub enum CpError {
    ...
    #[error("{}", translate!("cp-error-option-not-implemented", "option" => .0.as_str()))]
...
impl From<preserve::PreserveError> for CpError {
    fn from(e: preserve::PreserveError) -> Self {
        match e {
            preserve::PreserveError::Io(io) => Self::IoErr(io),
            preserve::PreserveError::IoContext(io, ctx) => Self::IoErrContext(io, ctx),
            preserve::PreserveError::Other(s) => Self::Error(s),
        }
    }
}
...
mod options {
    ...
    pub const PRESERVE_ALL: &str = "preserve-all";
    pub const SD: &str = "sd";
    pub const SD_EXPLICIT: &str = "sd-explicit";
...
pub fn uu_app() -> Command {
    ...
    Command::new("cp")
        ...
        .arg(
            Arg::new(options::PRESERVE_ALL)
                .long(options::PRESERVE_ALL)
                .help(translate!("cp-help-preserve-all"))
                .action(ArgAction::SetTrue),
        )
        .arg(
            Arg::new(options::SD)
                .long(options::SD)
                .help(translate!("cp-help-sd"))
                .action(ArgAction::SetTrue),
        )
        .arg(
            Arg::new(options::SD_EXPLICIT)
                .long(options::SD_EXPLICIT)
                .help(translate!("cp-help-sd-explicit"))
                .action(ArgAction::SetTrue),
        )
        .arg(
...
impl Options {
    ...
    fn from_matches(matches: &ArgMatches) -> CopyResult<Self> {
        ...
        // Resolve the `--preserve` family. `cp` follows POSIX overriding
        // conventions (later flags win; `-a` expands to `-dR --preserve=all`);
        // `preserve::resolve` reproduces that by ordering on the clap index.
        let attributes = preserve::resolve(
            matches,
            &[
                (options::PRESERVE, preserve::PreserveOpt::Preserve),
                (options::NO_PRESERVE, preserve::PreserveOpt::NoPreserve),
                (options::ARCHIVE, preserve::PreserveOpt::All),
                (
                    options::PRESERVE_DEFAULT_ATTRIBUTES,
                    preserve::PreserveOpt::Default,
                ),
                (options::PRESERVE_ALL, preserve::PreserveOpt::All),
                (options::SD, preserve::PreserveOpt::Sd),
                (options::SD_EXPLICIT, preserve::PreserveOpt::SdExplicit),
                (
                    options::NO_DEREFERENCE_PRESERVE_LINKS,
                    preserve::PreserveOpt::Links,
                ),
            ],
        )?;
    ...
    fn verify(self, path: &Path, debug: bool) -> CopyResult<()> {
        match self {
            ...
            Self::Interactive(_) => {
                ...
                let prompt_yes_result = prompt_yes!("{prompt_msg}");
...
fn copy_link(
    ...
    Ok(copy_attributes(source, dest, &options.attributes)?)
}
...
mod tests {
    ...
    #[test]
    fn test_diff_attrs_basic() {
        let all = Attributes::ALL;
        let diffed = all.diff(&Attributes::ALL);
        // Diffing ALL with ALL flips every field to No { explicit: true }
        ...
        assert!(matches!(diffed.timestamps, Preserve::No { explicit: true }));
        assert!(matches!(diffed.links, Preserve::No { explicit: true }));
        assert!(matches!(diffed.xattrs, Preserve::No { explicit: true }));
    }
}
