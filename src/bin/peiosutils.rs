...
{indent_list}"
    ...
    {
        let _ = writeln!(io::stderr(), "peiosutils: {}", strip_errno(&e));
        process::exit(1);
    }
}
...
fn main() {
    ...
    let is_peiosutils = binary_as_util.ends_with("utils");
    let matched_util = utils
        .keys()
        .filter(|&&u| binary_as_util.ends_with(u) && !is_peiosutils)
        .max_by_key(|u| u.len()); //Prefer stty more than tty. *utils is not ls

    let util_name = if let Some(&util) = matched_util {
        Some(OsString::from(util))
    } else if is_peiosutils || binary_as_util.ends_with("box") {
    ...
    if let Some(util_os) = util_name {
        ...
        match util {
            "--list" => {
                ...
                if args.next().is_some() {
                    let _ = writeln!(io::stderr(), "peiosutils: invalid argument");
                    process::exit(1);
                }
            ...
            "--version" | "-V" => {
                if let Err(e) = writeln!(io::stdout(), "peiosutils {VERSION} (multi-call binary)")
                    && e.kind() != io::ErrorKind::BrokenPipe
                {
                    let _ = writeln!(io::stderr(), "peiosutils: {}", strip_errno(&e));
                    process::exit(1);
        ...
        if binary_as_util.ends_with("box") {
            ...
            let _ = writeln!(io::stderr(), "peiosutils: missing argument");
        }
        process::exit(1);
    }
}
