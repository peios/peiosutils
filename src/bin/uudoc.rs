...
fn usage<T: Args>(utils: &UtilityMap<T>) {
    println!("uudoc - Documentation generator for peiosutils (Peios fork of uutils/coreutils)");
    println!();
...
/// Generates the peiosutils multiplexer app for the utility map
fn gen_peiosutils_app<T: Args>(util_map: &UtilityMap<T>) -> Command {
    let mut command = Command::new("peiosutils");
...
fn gen_manpage<T: Args>(
    ...
    let command = if utility == "peiosutils" {
        gen_peiosutils_app(util_map)
    } else {
...
fn gen_completions<T: Args>(args: impl Iterator<Item = OsString>, util_map: &UtilityMap<T>) -> ! {
    ...
    let mut command = if utility == "peiosutils" {
        gen_peiosutils_app(util_map)
    } else {
        validation::setup_localization_or_exit(utility);
